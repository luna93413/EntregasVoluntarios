...
void inicializar_potencial_multibarrera(double *V, double lambda, double k_tilde, int n_barreras, int *inicio_detector) {
    ...
    for (int b = 0; b < n_barreras; b++) {
        inicio = offset+100 + b * (ancho + separacion);
        fin = inicio + ancho - 1;
        if (fin > N) break;
...
// Encuentra el primer máximo global de un array PD de tamaño T
...
int main() {
    ...
    int offset = (int)(N * h / 4.0 / h) + N/16 +10; 
